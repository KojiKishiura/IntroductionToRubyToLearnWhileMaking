...
# 検索の処理
# http://localhost:8099/retrieveで呼び出される
...
# 修正の処理
# http://localhost:8099/editでよびだされる
server.mount_proc("/edit") {|req, res|
  ...
  dbh.disconnect
  # 処理の結果を表示する
  # ERBをERBHandlerを経由せずに直接呼び出して利用
  template = ERB.new(File.read('edited.erb'))
  res.body << template.result(binding)
}

# 削除の処理
# http://localhost:8099/deleteでよびだされる
