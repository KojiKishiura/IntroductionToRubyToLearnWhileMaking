  ...
  def findBookInfos
    ...
    if year == 0 or month == 0 or day == 0
     ...
     @book_infos.each {|key, info|
      output = true
      unless fkey == "" 
        output = false unless fkey == key 
      end
      unless title == "" 
        output = false unless title == info.title
      end
      unless author == ""
        output = false unless author == info.author
      end
      unless page == 0 
        output = false unless page == info.page
      end
      unless publish_date == nil 
        output = false unless publish_date == info.publish_date
      end
      print info.toFormattedString if output
      puts "\n---------------"
    }
  end
